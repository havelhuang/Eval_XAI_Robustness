...
def greyscale_multilevel_uniform(
    ...
    for level_idx in range(count_max_levels):
        ...
        cov_square += (1-count_keep.float()/count_particles)/count_keep.float()
        # print('term', torch.log(count_keep.float()).item() - math.log(count_particles))

        # Early termination
        if lg_p < -100:
            return -100., cov_square, x, levels
        ...
        L_prev = L
        # input()
    return lg_p, cov_square, x, levels  # , l_inf_min
